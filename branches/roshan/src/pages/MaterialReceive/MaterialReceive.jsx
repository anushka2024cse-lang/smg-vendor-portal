        ...
        <div className="p-6 text-slate-300 w-full">
            <div className="flex items-center gap-4 mb-6">
                <div>
                    ...
                    <h1 className="text-2xl font-bold text-white">Material Receive</h1>
                </div>
            </div>

            {/* Add New Material Receipt Form */}
            <div className="bg-[#1f2533] p-6 rounded-xl border border-slate-800 mb-8">
                <h2 className="text-lg font-semibold text-white mb-2">Add New Material Receipt</h2>
                <p className="text-sm text-slate-400 mb-6">Log a new incoming material shipment. This will automatically update inventory levels.</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    ...
                    <div>
                        <label className="block text-sm font-medium mb-1 text-slate-400">Date of Receiving</label>
                    ...
                    <div>
                        <label className="block text-sm font-medium mb-1 text-slate-400">No. of Units Received</label>
                    ...
                    <div>
                        <label className="block text-sm font-medium mb-1 text-slate-400">Received By</label>
                        <input type="text" className="w-full bg-[#1E293B] border border-slate-700 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-blue-500 text-sm" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium mb-1 text-slate-400">Received At (Location)</label>
                        <input type="text" placeholder="e.g. Warehouse A" className="w-full bg-[#1E293B] border border-slate-700 rounded-lg px-4 py-2 text-white placeholder-slate-500 focus:outline-none focus:border-blue-500 text-sm" />
                    </div>

                    <div>
                        <label className="block text-sm font-medium mb-1 text-slate-400">Logistics Company</label>
                        <input type="text" placeholder="e.g. Speedy Logistics" className="w-full bg-[#1E293B] border border-slate-700 rounded-lg px-4 py-2 text-white placeholder-slate-500 focus:outline-none focus:border-blue-500 text-sm" />
                    </div>
                    <div>
            ...
            <div className="bg-[#1f2533] rounded-xl border border-slate-800 overflow-hidden">
                <div className="p-6 border-b border-slate-800 flex justify-between items-center">
                    <div>
                        <h2 className="text-lg font-semibold text-white mb-1">Receive History</h2>
                        <p className="text-sm text-slate-400">A log of all past material receipts.</p>
                    </div>
                    ...
                    <table className="w-full text-sm text-left whitespace-nowrap">
                        <thead className="text-xs text-slate-400 uppercase bg-[#1E293B]/50 border-b border-slate-800">
                            <tr>
                                ...
                                <th className="px-4 py-3 font-semibold opacity-70">Date Rcvd.</th>
                                ...
                                <th className="px-4 py-3 font-semibold opacity-70">Received By</th>
                                <th className="px-4 py-3 font-semibold opacity-70">Rcvd. At</th>
                                <th className="px-4 py-3 font-semibold opacity-70">Logistics</th>
                        ...
                        <tbody className="divide-y divide-slate-800/50">
                            {history.length > 0 ? history.map((item, index) => (
                                <tr key={item.id} className="hover:bg-white/5 transition-colors group">
                                    ...
                                    <td className="px-4 py-3 text-slate-300">{item.dateReceived}</td>
                                    ...
                                    <td className="px-4 py-3 text-slate-300">{item.receivedBy}</td>
                                    <td className="px-4 py-3 text-slate-300">{item.receivedAt}</td>
                                    <td className="px-4 py-3 text-slate-300">{item.logistics}</td>
